...
def get_plotting_params(caps_2d, cap_3d, ang_names=[]):
    ...
    width_2d = max([round(p['width'] * height_2d / p['height']) for p in params_2d])

    param_3d = get_video_params_cap(cap_3d)
    height_3d = param_3d['height']
    width_3d = param_3d['width']

    # Calculate grid layout for 2x4 videos
    n_cols = 2
    n_rows = 4
    grid_width = n_cols * width_2d + (n_cols - 1) * spacing_videos
    grid_height = n_rows * height_2d + (n_rows - 1) * spacing_videos

    start_3d = grid_height + spacing_videos
    start_angles = start_3d + height_3d + spacing_videos + spacing_angle

    width_total = max(grid_width, width_3d)
    height_total = grid_height + spacing_videos + height_3d + spacing_videos + \
...
def draw_data(start_img, frames_2d, frame_3d, all_angles, pp):
    ...
    # Resize frames for the grid
    frames_2d_resized = [cv2.resize(f, (width_2d, height_2d)) for f in frames_2d]

    imout = np.copy(start_img)

    # Arrange 2D videos in a 2x4 grid
    for i, frame in enumerate(frames_2d_resized):
        row = i // n_cols
        col = i % n_cols
        x_start = col * (width_2d + spacing_videos)
        y_start = row * (height_2d + spacing_videos)
        imout[y_start:y_start + height_2d, x_start:x_start + width_2d] = frame

    # Place the 3D video below the grid
